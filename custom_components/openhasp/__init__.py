...
async def async_setup(hass, config):
    ...
    if conf is None:
        # We still depend on YAML so we must fail if missing
        _LOGGER.error(
...
class SwitchPlate(RestoreEntity):
    ...
    async def async_added_to_hass(self):
        ...
        # Load pages_schema.json asynchronously to avoid blocking
        ...
        async def statusupdate_message_received(msg):
            ...
            try:
                ...
                # Update Plate device info (firmware version, etc.)
